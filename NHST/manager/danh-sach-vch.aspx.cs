        ...
        public void pagingall(List<tbl_TransportationOrderNew> acs)
        {
            ...
            if (acs.Count > 0)
            {
                ...
                for (int i = FromRow; i < ToRow + 1; i++)
                {
                    ...
                    {
                        hcm.Append("<td>" + string.Format("{0:dd/MM/yyyy HH:mm}", Convert.ToDateTime(item.DateInTQWareHouse)) + "</td>");
                    }
                    else
                    {
                        hcm.Append("<td></td>");
                    ...
                    if (ac.RoleID == 0)
                        hcm.Append("<td><a class=\"btn btn-info btn-sm green\" href=\"/manager/Edit-Transportation.aspx?id=" + item.ID + "\" >Sửa</a></td>");
                    hcm.Append("</tr>");
                }
